...
# genObject()
{
	local (*Everything::HTML::FormObject::Checkbox::getParamArray,
	...
	*Everything::HTML::FormObject::Checkbox::SUPER::genObject = sub {
	...
	is( $params[0], 'query, bindNode, field, name, checked, unchecked, default, label'.
			" $node bN f n c u d l",
		'genObject() should call getParamArray() with @_' );
	is( $node->{_calls}[0][0], 'genObject',
		'... and SUPER::genObject()' );
	is( $node->{_calls}[1][0], 'checkbox',
		'... and $q->checkbox()' );
	like( $node->{_calls}[0][3], qr/^.+:u$/,
		'... uses provided $unchecked' );
	like( $node->{_calls}[0][3], qr/^f:.+$/,
		'... and $field' );
	is( $node->{_calls}[0][4], 'n',
		'... and $name' );
	is( $node->{_calls}[1][4], 'd',
		'... and $default' );
	is( $node->{_calls}[1][6], 'c',
		'... and $checked' );
	is( $node->{_calls}[1][8], 'l',
		'... and $label' );
	is( $result, "html\na",
	...
	is( $node->{_calls}[-1][6], 1,
 	...
 	is( $node->{_calls}[-1][4], 0,
		'... with no $bindNode, $default defaults to 0' );
...
is( join( '', @{ $node->{_calls}[-3] } ), 'paramname',
	'cgiUpdate() should call param() with $name' );
is( join( '', @{ $node->{_calls}[-2] } ), "getBindField${node}name",
	'... should call getBindField() with $query, $name' );
is( join( '', @{ $node->{_calls}[-1] } ), 'verifyFieldUpdatefield',
...
is( $node->{field}, 'var',
	'... and to $unchecked if not' );

push @results, cgiUpdate( $node, $node, 'name', $node, 0 );
is( $results[2], 0,
	'... should return 0' );
is( $results[1], 1,
	'... unless $overrideVerify is true' );
is( $results[0], 1,
	'... or $NODE->verifyFieldUpdate($field) is true' );
