...
my (
	$le_wrn,  $le_err,     $ms_isclosed, $ms_params, $ms_gotsettings,
	$ms_addr, $ms_subject, $ms_body,     $ms_from
);
...
'node2mail should log a warning if sending an email with a body with all spaces'
	...
	node2mail( $email, 6 );
	ok( $ms_gotsettings,
'node2mail should request getNode(\'mail settings\', \'setting\') on valid body and subject (no valid getVars)'
	);
	like(
		join( "", @WARNINGS ),
...
'node2mail should default to send from root@localhost if none is specified'
	...
	node2mail( $email, 6 );
	ok( $ms_gotsettings,
'node2mail should request getNode(\'mail settings\', \'setting\') on valid body and subject (valid getVars)'
	);
	ok( $ms_params->{smtp} eq "localhost",
	...
	node2mail( $email, 6 );
	ok(
		$ms_params->{smtp} eq "mymailserver",
...
'node2mail should use $SETTINGS->{systemMailFrom} if in partial mail settings'
	...
	ok( $ms_body eq "test body",
		'node2mail calls MailMsg with the correct $body' );
	ok( $ms_subject eq "test title",
		'node2mail calls MailMsg with the correct $subject' );
	ok( $ms_addr eq $email, 'node2mail calls MailMsg with the correct $addr' );
	is(
		$ms_params->{from},
		'root@nowhereinparticular.com',
	...
	ok(
		( join "", @$arraymembers ) eq ( join "", @RECIPIENTS ),
...
ok( ( join "", @WARNINGS ) =~ /No input files for mail2node/,
...
ok( ( join "", @WARNINGS ) =~ /doesn\'t appear to be a valid mail file/,
	'...and should throw a warning saying so' );
...
mail2node('/dummy/file');
ok( join( "", @ERRORS ) =~ /mail2node\: Node creation of type mail failed\!/,
