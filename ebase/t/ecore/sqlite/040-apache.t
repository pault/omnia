...
my $fh = IO::File->new( 't/conf/extra.conf.in', 'w' ) or die "Can't open file for writing, $!";
...
my $base = 'http://' . Apache::Test::vars->{remote_addr} . ':' . Apache::Test::vars->{port};
...
$mech->title_is( 'Permission Denied', '...redirected to not allowed node.');

## try to login as root

$mech->get_ok( "$base?node=login", '.... login node OK.' );
$mech->title_is("login", '...not redirected.');
$mech->submit_form_ok(  { form_number => 3, fields => { user => 'root' }, button => 'submit' }, '...submits form.' );
$mech->content_contains("Hey.  Glad you're back", '... reports we are logged in.' ) || diag $mech->content;
...
$mech->follow_link_ok( { text => "Edit root's settings" }, "Go to our setting's page.");
$mech->submit_form_ok ( { form_number => 5, fields => {passwd => 'root', passwd_confirm => 'root'}, button => 'button' }, "...and change root password.");

$mech->get_ok( "$base?node=login", 'We go back to the login page.' );
$mech->submit_form_ok(  { form_number => 3, fields => { user => 'root', passwd => 'root' }, button => 'submit' }, '...login with our new password.' );
$mech->content_contains("Hey.  Glad you're back", '...and are presented with the welcome message.' ) || diag $mech->content;

## Now let's create another user
my $user_type = $nb->getNode( 'user', 'nodetype' );
$mech->follow_link_ok( { text => "Create new node" }, "Let's go to the create ne node page.");
$mech->submit_form_ok(  { form_number => 5, fields => { node => 'a user', type => $user_type->getId }, button => 'createit' }, '...create a new user.' );

$mech->follow_link_ok( { text => 'edit' }, '...and go to the edit page.' );
$mech->submit_form_ok ( { form_number => 5, fields => {passwd => 'password', passwd_confirm => 'password'}, button => 'button' }, "...and change the user's password.");
$mech->follow_link_ok( { text => "Log root out" }, "...and logout root out.");

### Now log in as 'a user' and do access tests

$mech->get_ok( "$base?node=login", 'We go back to the login page.' );
$mech->submit_form_ok(  { form_number => 3, fields => { user => 'a user', passwd => 'password' }, button => 'submit' }, '...login with our new password.' );
$mech->content_contains("Hey.  Glad you're back", '...and are presented with the welcome message.' ) || diag $mech->content;

$mech->get_ok( "$base/node/2", 'A user goes to the node nodetype.' );
$mech->title_is( 'Permission Denied', '...and is redirected to not allowed node.');
