...
is(isSameVersion(), undef, 'isSameVersion() should return undef without node');
...
$node->{node_id} = 13;
ok( !isSameVersion($mock, $node), '... false unless node version is verified' );

$node->{node_id} = 12;
$mock->set_series( getGlobalVersion => undef, 2, 1 );
ok( !isSameVersion($mock, $node), '... false unless node has global version' );
