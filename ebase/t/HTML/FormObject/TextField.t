...
{
	local (
		*Everything::HTML::FormObject::TextField::getParamArray,
	...
	*Everything::HTML::FormObject::TextField::SUPER::genObject = sub {
	...
	is(
		$params[0],
		'query, bindNode, field, name, default, size, maxlen ' . $node
			. ' bN f n d s m',
		'genObject() should call getParamArray() with @_'
	);
	is( $node->{_calls}[0][0],
		'genObject', '... should call SUPER::genObject()' );
	is( $node->{_calls}[1][0], 'textfield', '... should call textfield()' );
	is( $node->{_calls}[1][4],
		'd', '... should use default value, if provided' );
	is( $node->{_calls}[1][2], 'n', '... should use provided name' );
	is( $node->{_calls}[1][6], 's', '... should use provided size' );
	is( $node->{_calls}[1][8], 'm', '... should use provided maxlen' );
	is( $result, "html\na",
	...
	is( $node->{_calls}[-1][6], 20,  '... size should default to 20' );
	is( $node->{_calls}[-1][8], 255, '... maxlen should default to 255' );
	is( $node->{_calls}[-1][2],
