...
{
	local (
		*Everything::HTML::FormObject::TextArea::getParamArray,
	...
	*Everything::HTML::FormObject::TextArea::SUPER::genObject = sub {
	...
	is(
		$params[0],
		'query, bindNode, field, name, default, cols, rows, wrap ' . $node
			. ' bN f n d c r w',
		'genObject() should call getParamArray() with @_'
	);
	is( $node->{_calls}[0][0],
		'genObject', '... should call SUPER::genObject()' );
	is( $node->{_calls}[1][0], 'textarea', '... should call textarea()' );
	is( $node->{_calls}[1][4],
		'd', '... should use default value, if provided' );
	is( $node->{_calls}[1][2],  'n', '... should use provided name' );
	is( $node->{_calls}[1][6],  'c', '... should use provided cols' );
	is( $node->{_calls}[1][8],  'r', '... should use provided rows' );
	is( $node->{_calls}[1][10], 'w', '... should use provided wrap' );
	is( $result, "html\na",
	...
	is( $node->{_calls}[-1][6], 80, '... cols should default to 80' );
	is( $node->{_calls}[-1][8], 20, '... rows should default to 20' );
	is( $node->{_calls}[-1][10],
		'virtual', '... wrap should default to "virtual"' );
	is( $node->{_calls}[-1][2],
		'f', '... name should default to node field name' );
