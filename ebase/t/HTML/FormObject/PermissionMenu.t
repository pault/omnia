...
{
	local (
		*Everything::HTML::FormObject::PermissionMenu::getParamArray,
	...
	*Everything::HTML::FormObject::PermissionMenu::SUPER::genObject = sub {
	...
	is( $node->{_calls}[1][0], 'addHash',      '... should call addHash()' );
	is( $node->{_calls}[2][0], 'addHash',      '... and addHash()' );
	is( $node->{_calls}[3][0], 'addHash',      '... and addHash() once again' );
	is( $node->{_calls}[4][0], 'genPopupMenu', '... and genPopupMenu()' );
	is( $node->{_calls}[4][2], 'n', '... should use provided $name' );
	is( $node->{_calls}[4][3],
		undef, '... $default becomes undef if true and not "AUTO"' );
	is( $result, "html\na",
...
# cgiUpdate()
{
	...
	is( $node->{_calls}[0][0], 'param', 'cgiUpdate() should call param()' );
	is( $node->{_calls}[1][0], 'getBindField', '... and getBindField()' );
	is( $node->{_calls}[2][0],
		'verifyFieldUpdate',
