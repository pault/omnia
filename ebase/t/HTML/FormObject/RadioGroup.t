...
{
	local (
		*Everything::HTML::FormObject::RadioGroup::getParamArray,
	...
	*FakeNode::radio_group = sub {
	...
	is(
		$params[0],
		"query, bindNode, field, name, default, vertical $node " . 'bN f n d v',
		'genObject() should call getParamArray() with @_'
	);
	is( $node->{_calls}[0][0],
		'genObject', '... should call SUPER::genObject()' );
	is( $node->{_calls}[1][0],
		'getValuesArray', '... should call getValuesArray()' );
	is( $node->{_calls}[2][0],
		'getLabelsHash', '... should call getLabelsHash()' );
	is( $node->{_calls}[3][0],
		'radio_group', '... should call $query->radio_group()' );

	genObject( $node, $node, { f => 'field' }, 'f', 'n', 'AUTO' );
	is( $node->{_calls}[-1][4],
		'field', '... default value should bind to node field if "AUTO"' );
	...
	is( genObject( $node, $node, 'bN', 'f', 'n', 'd', 0 ),
		"html\na\nb", '... join buttons using "\n" if vertical is false' );
	ok(
		is(
			genObject( $node, $node, 'bN', 'f', 'n', 'd', 1 ),
			"html\na<br>\nb",
			'... and join using "<br>\n" if vertical is true'
		),
