...

{
	...
	my @imports;
	*Everything::import = *Everything::HTML::FormObject::Checkbox::import =
		sub {
		push @imports, $_[0];
		};
...
{
	local (
		*Everything::HTML::FormObject::RemoveVarCheckbox::getParamArray,
	...
	*Everything::HTML::FormObject::RemoveVarCheckbox::SUPER::genObject = sub {
	...
	is(
		$params[0],
		"query, bindNode, field, var $node bN f v",
		'genObject() should call getParamArray() with @_'
	);

	is( $node->{updateExecuteOrder},
		55, '... should set node execution order to 55' );

	my $call = $node->{_calls}[0];
	is( $call->[0], 'genObject', '... should call SUPER::genObject()' );
...
is( $node->{_calls}[1][0],
	'getBindField', '... should call getBindField() to find field' );
isnt( $node->{_calls}[2][0],
	'verifyFieldUpdate',
