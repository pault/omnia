...
*Everything::Security::import = 
	*Everything::Util::import = 
...
# fieldToXML()
{
	...
	*fieldToXML = \&Everything::Node::setting::fieldToXML;
	...
	is( fieldToXML($node, '', '', '!'), 2, 
		'fieldToXML() should delegate to SUPER() unless field param is "vars"');

	$node->{_calls} = [];
	is( fieldToXML($node, '', 'vars'), $node,
		'... should return XML::DOM element for vars, if "vars" field' );
...
# xmlTag()
{
	...
	*Everything::Node::setting::parseBasicTag = sub {
	...
	is( Everything::Node::setting::xmlTag($node, $node), 3,
	...
	is( $node->{vars}{$fixes->[0]{where}}, -1,
		'... and should mark fixable nodes by name in "vars"' );
...
is( Everything::Node::setting::getNodeKeepKeys($node), $node,
	'getNodeKeepKeys() should call SUPER()' );
...
is( Everything::Node::setting::updateFromImport($node, $node), 10,
