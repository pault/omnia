...
{
	local (
		*Everything::NodeBase::subclass::databaseConnect,
	...
	*Everything::NodeBase::subclass::getNode = sub {
	...
	is( join( '-', @{ $dbc[0] } ),
		"$result-db-h-u-p", '... passing db connect options' );
...
$mock->set_always( countNodeMatches => 20 )
	...
	->set_series( execute => 0, (1) x 3 )->set_series( fetchrow => 0 .. 3 )
	->set_true('finish')->clear();
...
$mock->set_series( getNodeCursor => 0, ($mock) x 2 )
	->set_series( execute => 0, 1 )->set_always( fetchrow => 33 )
	->set_true('finish')->set_true('finish')->clear();

$result = countNodeMatches( $mock, 'w', 't' );
( $method, $args ) = $mock->next_call();
is( $method, 'getNodeCursor', 'countNodeMatches() should get a db cursor' );
is( join( '-', @$args ),
	"$mock-count(*)-w-t", '... counting and passing conditions' );
...
$mock->set_series( sqlSelectMany => undef, $mock )
	...
	->set_series( getNode => 'a', 'b', 'c' )->set_true('finish')->clear();
...
is(
	join( '-', @$args ),
	"$mock-node_id-node-type_nodetype=1",
...
can_ok( $package, 'dropNodeTable' );
{
	...
	*Everything::printLog = sub {
	...
	$mock->set_series( tableExists => 0, 1 )
		->set_always( genTableName => 'tname' )->set_always( do => 'done' )
		->clear();
...
$mock->set_always( quote => 'quoted' )->clear();
...
is( buildNodetypeModules($mock),
	undef, 'buildNodetypeModules() should return with no database cursor' );
...
$mock->set_always( 'genTableName', 'table name' )
	->set_always( 'prepare', $mock )->set_always( 'execute', 'executed' )
	->clear();
...
$mock->clear()->set_series( 'sqlSelectMany', undef, ($mock) x 3 )
	->mock( 'fetchrow', sub { return @{ shift @frargs } } )->set_true('finish');
...
$mock->clear()->set_always( 'genTableName', 'gentable' )
	->set_series( 'prepare', ($mock) x 2, 0 )->set_series( 'execute', 1, 0 );

my $joins = { one => 1, two => 2 };
$result =
	sqlSelectJoined( $mock, 'select', 'table', $joins, 'where', 'other',
	'bound', 'values' );
...
is(
	( $mock->next_call() )[1]->[1],
	'SELECT sel FROM gentable ',
	'... using it in the SQL statement'
);
sqlSelectMany( $mock, 'sel', '', 'whe' );
is(
	( $mock->next_call(2) )[1]->[1],
	'SELECT sel WHERE whe ',
...
$mock->clear()->set_series( 'sqlSelectMany', 0, $mock )
	->set_always( 'fetchrow_hashref', 'hash' )->set_true('finish');

$result = sqlSelectHashref( $mock, 'foo', 'bar', 'baz', 'quux', 'qiix' );
( $method, $args ) = $mock->next_call();
is( $method, 'sqlSelectMany',
	'sqlSelectHashref() should call sqlSelectMany()' );
is( join( '-', @$args ), "$mock-foo-bar-baz-quux-qiix",
	'... passing all args' );
...
is(
	$args->[1],
	"INSERT INTO gentable (n, m, s) VALUES(?, 1, 8)",
	'... with names and values quoted appropriately'
);
is_deeply( $args->[2], ['foo'], '.. and bound args as appropriate' );
...
can_ok( $package, 'sqlExecute' );
{
	...
	*Everything::printLog = sub { $log = shift };
	...
	$mock->clear()->set_series( 'prepare', $mock, 0 )
