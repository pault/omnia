...
# insert()
{
	...
	*Everything::logErrors = sub { $error = shift };
	$mock->set_true('setVars')->set_series( SUPER => 0, 1, 0 )->set_series(
		getNode => '',
	...
	*insert = \&Everything::Node::nodeball::insert;
	...
	( $method, $args ) = $mock->next_call(3);
	is( $args->[1]->{author},
		'title', '... should respect given title when creating default vars' );
}

# getVars()
$mock->set_always( getHash => 10 )->clear();

is( Everything::Node::nodeball::getVars($mock),
	10, 'getVars() should call getHash()' );
...
is( Everything::Node::nodeball::setVars( $mock, 12 ),
	11, 'setVars() should call setHash()' );
( $method, $args ) = $mock->next_call();
...
# fieldToXML()
{
	...
	*Everything::Node::setting::fieldToXML = sub {
	...
	is( Everything::Node::nodeball::fieldToXML(@args),
		4, 'fieldToXML() should call SUPER() unless handling a "vars" field' );

	$args[2] = 'vars';
	is( scalar Everything::Node::nodeball::fieldToXML(@args),
		4, '... should delegate to setting nodetype if handling "vars" field' );
...
# xmlTag()
{
	...
	*Everything::Node::setting::xmlTag = sub {
	...
	$mock->set_always( SUPER => 1 )->set_series( getTagName => 0, 'vars' )
		->clear();

	is( Everything::Node::nodeball::xmlTag( $mock, $mock ),
		1, 'xmlTag() should call SUPER() unless XMLifying a "vars" field' );
	is( $mock->next_call(), 'getTagName', '... calling getTagName() on tag' );

	is( scalar Everything::Node::nodeball::xmlTag( $mock, $mock ),
		2, '... should delegate to settings node if passed "vars" field' );
...
# applyXMLFix()
{
	...
	*Everything::Node::setting::applyXMLFix = sub {
	...
	is( Everything::Node::nodeball::applyXMLFix(@args),
		18,
		'applyXMLFix() should call SUPER() unless fixing up "setting" field' );
	$fix->{fixBy} = 'setting';
	is( scalar Everything::Node::nodeball::applyXMLFix(@args),
		3,
