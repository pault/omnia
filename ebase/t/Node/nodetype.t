...
# construct()
$mock->set_true('SUPER')->set_true('finish');

$mock->{node_id} = $mock->{extends_nodetype} = 0;
$mock->{sqltable} = 'foo,bar,baz';

ok( construct($mock), 'construct() should always succeed (unless it dies)' );
...
$mock->set_always( sqlSelect => 1 )->set_always( sqlSelectJoined => $mock )
	->set_always( fetchrow_hashref => $mock )->clear();
...
is(
	join( '-', @$args ),
	"$mock-node_id-node-title='node' AND type_nodetype=1",
...
is( $mock->{derived_grouptable},
	'grouptable', '... should copy derived fields if they are inherited' );

# misleading, I know...
is( $mock->{defaultgroupaccess}, -1, '... but should not copy other fields' );
is( $ip, '1 100',
	'... should call inheritPermissions() for permission fields' );
is( $mock->{derived_sqltable},
...
insert($mock);
isnt( $mock->{extends_nodetype},
	12, '... and should not be allowed to extend itself' );

# update()
$mock->set_series( SUPER => undef, 47 )->set_true('flushCacheGlobal')->clear();
update($mock);
