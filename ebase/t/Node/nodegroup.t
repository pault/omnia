...
$mock->{node_id} = 111;
$mock->{DB}      = $mock;
$mock->set_always( isGroup => 'grouptable' )->set_true('createGroupTable')
	->set_series( sqlSelectMany => undef, $mock )
	->set_series( fetchrow => ( 1, 7, 9 ) )->set_true('finish')->clear();
...
#  updateFromImport()
$mock->set_always( SUPER => 6 )->clear();

is( updateFromImport( $mock, { group => 7 }, 'user' ),
	6, 'updateFromImport() should return result of SUPER() call' );
( $method, $args ) = $mock->next_call();
...
# updateGroup()
$mock->set_always( hasAccess => 0 )->clear();
...
$mock->{group} = [ 1, 2, 4, 8 ];
$mock->set_series( do => ( 1, 2 ) )->set_always( isGroup => 'gtable' )
	->set_true('hasAccess')->set_always( restrict_type => $mock->{group} )
	->set_series( selectGroupArray => ( [ 2, 4, 6, 10 ] ) )
	->set_true('sqlSelect')->set_true('sqlInsert')->set_true('sqlUpdate')
	->set_series( sqlDelete => ( 1, 2 ) )->set_true('groupUncache')->clear();
...
like(
	$errors,
	qr/Wrong number of group members deleted!/,
...
like(
	$args->[3],
	qr/gtable_id=411.+node_id=8.+rank/,
	'... with the correct arguments'
);
is( join( ' ', @{ $mock->{group} } ),
	'1 2 4 8', '... and should assign new group to the node' );

# nuke()
$mock->{node_id} = 7;
$mock->{dbh}     = $mock;

$mock->set_always( SUPER => 12 )->set_always( isGroup => 'table' )
...
$mock->set_series( getId => 17, 44 )->set_true('groupCache')
...
# selectNodegroupFlat()
$mock->{flatgroup} = 17;
is( selectNodegroupFlat($mock),
	17, 'selectNodegroupFlat() should return cached group, if it exists' );
delete $mock->{flatgroup};

my $traversed = { $mock->{node_id} => 1, };
is( selectNodegroupFlat( $mock, $traversed ),
...
is( $mock->next_call(), 'isGroup', '... checking if node is a group node' );
...
ok( !removeFromGroup( $mock, 'user' ),
	'... or if no insertables are provided' );

$mock->set_series( hasAccess => ( 0, 1 ) )->clear();

ok(
	!removeFromGroup( $mock, 6, 'user' ),
	'... or if user does not have write access'
);
( $method, $args ) = $mock->next_call();
...
$mock->set_always( hasAccess => (1) )->set_always( getId => (6) )->clear();
...
# replaceGroup()
$mock->set_series( isGroup => ( '', 'table' ) )->set_series( hasAccess => 0, 1 )
...
# getNodeKeys()
$mock->set_series( SUPER => { foo => 1 }, { foo => 2 } )->clear();
...
# fieldToXML()
$mock->set_series( SUPER => ( 5, 6, 7 ) )->set_true('appendChild')->clear();
...

{
	...
	*Everything::XML::parseBasicTag = sub {
	...
	is( join( '|', @{ $inserts[0] } ),
		"$mock|-1|-1|0", '... as a dummy node if a where clause is provided' );
	is( join( '|', @{ $inserts[1] } ),
		"$mock|-1|node|1", '... or by name if a name is provided' );

	ok( !xmlTag( $mock, $mock ), '... should return nothing with no fixups' );
}

# applyXMLFix()
$mock->set_always( SUPER => 14 )->clear();
...
# clone()
$mock->set_series( SUPER => undef, ($mock) x 2 )->set_true('update')->clear();
...
# getNodeKeepKeys()
$mock->set_series( SUPER => { keep => 1, me => 1 } )->clear();
...
$mock->set_always( SUPER => 11 )->clear();

my $group = { group => [ 1, 4, 6 ] };
is( conflictsWith( $mock, $group ),
