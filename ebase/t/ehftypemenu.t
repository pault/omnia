...
# genObject()
{
	local (*Everything::HTML::FormObject::TypeMenu::getParamArray,
	...
	*Everything::HTML::FormObject::TypeMenu::SUPER::genObject = sub {
	...
	my $result = genObject( $node, 'q', 'bN', 'f', 'n', 't', 'd', 'U', 'p', 'n',
		'i', 'it' );

	is( $params[0], 'query, bindNode, field, name, type, default, USER, perm, '.
	...
	is( $node->{_calls}[1][0], 'addTypes',
		'... should call addTypes()' );
	is( $node->{_calls}[2][0], 'genPopupMenu',
		'... should call genPopupMenu()' );
	is( $node->{_calls}[2][2], 'n', 
		'... should use provided $name' );
	is( $node->{_calls}[1][1], 't', 
		'... should use provided $type' );
	is( $node->{_calls}[1][2], 'U',
		'... should use provided $USER' );
	is( $node->{_calls}[1][3], 'p',
		'... should use provided $perm' );
	is( $node->{_calls}[2][3], undef,
		'... $default becomes undef if true' );
	is( $result, "html\na", 	
		'... returning concatenation of SUPER() and genPopupMenu() calls' );

	genObject( $node, 'q', { f => 'field' }, 'f' );

	is( $node->{_calls}[-2][1], 'nodetype',
		'... $type should default to "nodetype"' );
	is( $node->{_calls}[-2][2], '-1',
		'... $USER should default to -1' );
	is( $node->{_calls}[-2][3], 'r',
		'... $perm should default to "r"' );
...
# addTypes()
{
	...
	is( $node->{_calls}[0][0], 'addHash',
		'addTypes() should call addHash() if defined $none' );
	is( $node->{_calls}[1][0], 'addHash',
		'... should call addHash() again if defined $inherit' );
	is( $node->{_calls}[2][0], 'addType',
		'... should call addType()' );
	is( $node->{_calls}[2][2], 'U',
		'... should use provided $USER' );
	is( $node->{_calls}[2][3], 'p',
		'... and $perm' );
	is( ${$node->{_calls}[1][1]}{'inherit (it)'}, 'i',
		'... $label should be set to "inherit ($inherittxt)" if $inherittxt' );
	is( $result, 1,
		'... should return 1' );

	addTypes ( $node, 't', '', '', 'n' );
	is( $node->{_calls}[-1][2], -1,
		'... $USER defaults to -1' );
	is( $node->{_calls}[-1][3], 'r',
		'... and $perm to "r"' );
	is( ${$node->{_calls}[-2][1]}{None}, 'n',
		'... skip an addHash() if $inherit undefined' );

	addTypes ( $node, 't', 'U', 'p', undef, 'i' );
	is( $node->{_calls}[-3][0], 'addType',
		'... skip and addHash() if $none undefined' );
	is( ${$node->{_calls}[-2][1]}{inherit}, 'i',
