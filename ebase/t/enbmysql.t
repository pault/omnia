...
is( join('-', @{$dconn[1]}), 'DBI-DBI:mysql:1:2-3-4',
...
$mock->set_always( 'getNode', $mock )
	 ->set_always( 'prepare_cached', $mock )
	 ->set_true( 'execute' )
	 ->set_series( 'fetchrow_hashref', @$fields );

my @result = Everything::NodeBase::mysql::getFieldsHash( $mock, 'table' );
($method, $args) = $mock->next_call();
is( $method, 'getNode', 'getFieldsHash() should fetch node' );
is( join('-', @$args[1, 2]), 'table-dbtable', '... by name, of dbtable type' );
($method, $args) = $mock->next_call();
...
ok( ! Everything::NodeBase::mysql::tableExists( $mock, 'target' ),
	'... returning false if table name is not found' );

can_ok( $package, 'createNodeTable' );
$mock->clear();
$mock->set_series( 'tableExists', 1, 0 )
	 ->set_always( 'do', 'done' );
...
like( $args->[1], qr/create table elbat/, '.. of the right name' );
like( $args->[1], qr/elbat_id int4/, '... with an id column' );
like( $args->[1], qr/rank .+node_id .+orderby/s,
...
is( $args->[1], 'alter table t drop f',
	'... altering the table, dropping the column' );
is( $result, 'done', '... returning the results' );

can_ok( $package, 'addFieldToTable' );
ok( ! Everything::NodeBase::mysql::addFieldToTable( $mock, '' ),
