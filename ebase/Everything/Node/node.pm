...
sub applyXMLFix
{
	...
	{
		print "Error! Fix not generated by 'node'.\n" if($printError);
		return $FIX;
	}

	my $TYPE;
	my $type = $$FIX{type_nodetype};

	# Depending on the order in which the nodes are imported, the type
	# may be an id, or a 'title,type' string combo.
	if($type =~ /^\d+$/)
	{
		$TYPE = $$this{DB}->getNode($type);
	}
	elsif($type =~ /^(.+?),(.+)$/)
	{
		$TYPE = $$this{DB}->getNode($1, $2);
	}
	else
	{
		print "Error!  Unknown type in fix\n";
	}
	
	my $NODE = $$this{DB}->getNode($$FIX{title}, $TYPE);

	unless($NODE)
	{
		print "Error! Unable to find '$$FIX{title}' of type " .
			"'$$FIX{type_nodetype}' for field $$FIX{field} of node\n" .
