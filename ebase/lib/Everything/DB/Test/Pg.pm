...
sub test_count_node_matches : Test(+0) {
    my $self = shift;
    $self->add_expected_sql(
q|SELECT count(*) FROM "node" LEFT JOIN "lions" ON "node".node_id = "lions".lions_id LEFT JOIN "serpents" ON "node".node_id = "serpents".serpents_id WHERE foo='bar' AND type_nodetype=sometype |
    );
    $self->SUPER;
}
...
sub test_get_node_cursor : Test(+0) {
    my $self = shift;
    $self->add_expected_sql( q|SELECT fieldname FROM "node" LEFT JOIN "lions" ON "node".node_id = "lions".lions_id LEFT JOIN "serpents" ON "node".node_id = "serpents".serpents_id WHERE foo='bar' AND type_nodetype=sometype ORDER BY title LIMIT 1 OFFSET 2|);
    $self->SUPER;
